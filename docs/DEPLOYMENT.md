...
Each canonical workspace or isolated task worktree has a private SQLite FTS5 index under `/home/coder/.local/share/comote/code-index`. The browser's index status request and each Codex message perform an incremental scan of Git tracked/untracked non-ignored files; only new, changed, or removed documents update the database. A forced rebuild is available from **Changes → Codebase index → Refresh index**. Indexing is bounded to 5,000 files, 256 KiB per file, and 40 MiB per workspace. Explicit filters exclude dependency/build directories, lockfiles, environment files, credential/key stores, database files, symlinks, unsupported/binary files, and anything ignored by Git. Common hard-coded token, password, and private-key patterns are redacted before indexed content is stored. Database files are mode `0600` and never enter application repositories or GitHub. Codex receives only a bounded shortlist of matching paths and extracted structural names, with an instruction to verify original files before relying on them.
