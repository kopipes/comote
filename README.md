...
- Private per-workspace SQLite FTS5 codebase index with incremental refresh, a manual refresh control, and bounded relevant-file hints for Codex
...
The **Codebase index** card in Changes shows the index for the selected task workspace. Comote refreshes it after Git-visible changes and before each Codex request; **Refresh index** forces a full rebuild. The SQLite FTS5 database lives under private Comote state, outside the application repository. It indexes source and configuration up to bounded file/size limits while excluding dependencies, build output, ignored files, environment files, credentials, keys, databases, symlinks, binaries, and oversized files; common hard-coded token/password/private-key patterns are redacted before storage. Codex receives at most ten matching paths plus extracted symbol/import/route/schema/config names and is required to verify the original source before editing. This reduces unnecessary file discovery and prompt context, but the source files remain the sole source of truth.
